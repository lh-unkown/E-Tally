...
class ETallyRequestHandler(http.server.SimpleHTTPRequestHandler):
    ...
    def handle_api_get(self, path, query):
        ...
        try:
            ...
            elif path.startswith("/api/tally/"):
                vin = path.replace("/api/tally/", "").strip()
            ...
            elif path == "/api/update-approval/list":
                cursor.execute("""
            ...
            elif path == "/api/supervisor/pending-tallies":
                cursor.execute("""
    ...
    def handle_api_post(self, path, data):
        ...
        try:
            if path == "/api/login":
            ...
            elif path == "/api/external/manifest":
                res = sync_engine.import_vessel_manifest(data)
                return self.send_json(res)

            elif path == "/api/supervisor/approve-tally":
                vin = data.get("vin")
            ...
            elif path == "/api/admin/override-tally":
                vin = data.get("vin")
                action = data.get("action", "UNLOCK") # UNLOCK or DELETE
            ...
            elif path == "/api/admin/add-vessel":
            ...
            elif path == "/api/admin/add-chassis":
            ...
            elif path == "/api/update-approval/request":
                vin = data.get("vin")
                user_id = data.get("user_id", "Surveyor 298")
            ...
            elif path == "/api/update-approval/respond":
                ...
                return self.send_json({"success": False, "message": "Unknown API POST endpoint"}, 404)
...
def run_server():
    database.init_db()
    with socketserver.TCPServer(("", PORT), ETallyRequestHandler) as httpd:
        print(f"HIPG e-Tally System running at http://localhost:{PORT}")
        httpd.serve_forever()
